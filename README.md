...
the ceiling, and finds where the Markov assumption fails (aftershocks; a reload-dependent
large-event hazard) and where a variance gap survives eight candidate repairs. Everything here is
...
**What the variance gap is not — eight exclusions.** The 25–30% excess in
run-to-run scatter is the one quantitative failure left that is not the
slow variable, so it was pursued until the data ran out of candidates.
Each repair was run forward and scored on three metrics — the count Fano
factor, the CV of stress released per 0.1-strain window, and the
within-preparation scatter of τ (Figure 21, left):

1. *Size law* — the three Section-8 arms agree, including empirical
   resampling; not the law.
2. *Aftershock hazard*, p(u,τ)·m(n) with the lag multipliers of Figure 20:
   lifts the count Fano factor from 1.0 to 1.2 (data 1.3–1.6) and
...
3. *Grid resolution* — the empirical arm on a 42 × 42 grid (552 cells):
   CV 0.114 → 0.116, τ scatter −5%.
4. *Reloading (all events)* — the hazard as a function of the stress
   reloaded since the last event is flat beyond the 5-step aftershock
   spike (0.0043 → 0.0049 across r = 0.01–0.12 in the flow cells; < 3% of
   the event-indicator deviance), and a simulation with that renewal
   hazard changes nothing. The +0.57 size→gap correlation reported in an
   earlier draft of this section is real (the simulation gives +0.02 on
   the same statistic) but lives almost entirely in the aftershock regime:
   restricted to events larger than 10⁻³ it is +0.18 (Figure 21, right).
5. *Reloading (large events)* — there *is* a reload-to-threshold
   structure, hidden because half of all events are tiny aftershocks that
   reset the reloading clock: referenced to the last event larger than
   0.01, the large-event hazard at fixed cell is 1.52× higher (16–84%:
   1.36–1.69, 98 cells) after 0.1–0.3 of reloaded stress than after
   < 0.04, and the data's large events are more regular than the model's
   (Fano 0.51 vs 0.71 for s > 0.01, 0.45 vs 0.57 for s > 0.03). A
   ...
   that regularity (Fano 0.71 → 0.61) and again leaves the τ scatter and
   the stress-budget CV untouched.
6. *Size–reload coupling* — a large event does not release what was
   reloaded: corr(ln s, ln r_big) = +0.02 at fixed cell (model +0.00);
   the median large-event size is flat in reloaded stress.
7. *Size–size coupling* — consecutive large-event sizes are independent
   (−0.01; model +0.01).
8. *Relaxation of a stress fluctuation* — identical in data and model at
   every lag from 0.002 to 0.05 strain (mean-reversion slopes −0.18/−0.13
   at 0.002, −1.0/−1.0 at 0.05), and a trajectory's initial stress
   deviation is forgotten on the same schedule in both (correlation 0.77
   at γ = 0.05, ≈ 0 by γ = 0.1). The model's stationary τ fluctuation is
   simply 17% larger with the same restoring force.

So the excess is a driving-noise amplitude that survives every
sequence-level correlation the data can exhibit at fixed (u,τ). The one
thing the data show that the model lacks at the individual-trajectory
...
**Figure 21 — the variance gap survives every hazard repair.** Left: the
three variance metrics relative to the data for the Markov model, the
aftershock hazard, the renewal hazard (all events), the two-class reload
hazard, and the empirical-resampling arm on the 22 × 22 and 42 × 42 grids.
Right: median strain to the next event against the size of the last, each
cell normalised by its own median gap, data and the Markov simulation's own
event log; the dependence is confined to the aftershock regime.
