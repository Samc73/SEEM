...
the ceiling, and finds where the Markov assumption fails outright (aftershocks, and a
reload-to-threshold structure between events). Everything here is
...
**What the variance gap is, and is not.** Three candidate repairs were
tried against the same three metrics — the count Fano factor, the CV of
the stress released per window, and the within-preparation scatter of τ at
γ = 0.1 (Figure 21, left). An aftershock hazard p(u,τ)·m(n), with the
quiet-baseline field and the lag multipliers m(1…8) = 20, 7.3, 3.1, … taken
straight from Figure 20, lifts the Fano factor from 1.0 to 1.2 (data 1.3–
1.6) and, as a bonus, improves the yield peaks (RMS 2.5% → 1.6%) — but it
does nothing for the stress-budget variance (CV 0.13 → 0.13, data 0.11)
or the τ scatter. Doubling the grid to 42 × 42 (empirical-resampling arm,
552 cells) changes nothing either (CV 0.114 → 0.116; τ scatter −5%). So
the excess is not the size law (Section 8 arms agree), not the hazard's
short memory, and not resolution. It is a **renewal structure** the
Markov description cannot hold at any resolution: at fixed cell, the
strain to the next event is strongly tied to the size of the last one —
correlation +0.57 between ln s_k and ln(gap), and a 3.5× longer median
reloading after an event larger than 0.05 than after one smaller than
0.005 (Figure 21, right; 177 cells, 109 for the ratio) — whereas
consecutive *sizes* are nearly independent (+0.08). The glass reloads to
the next instability; the model re-rolls the dice every step. This is the
threshold-and-reload picture of elastoplastic models, and it is the
model-class change the data ask for next: a hazard that depends on the
stress *accumulated since the last event*, not only on the state.

![Renewal](figures/fig21_renewal.png)

**Figure 21 — the variance gap is a renewal structure.** Left: the three
variance metrics relative to the data for the Markov model, the same with
the aftershock hazard, and the empirical-resampling arm on the 22 × 22 and
42 × 42 grids. Right: median strain to the next event against the size of
the last, each cell normalised by its own median gap (a Markov model is
flat at 1 within a cell).
...
| The 25–30% variance excess is a size-law, hazard-memory or grid-resolution effect | **contradicted** for all three (Fig. 21): it is a renewal (reload-to-threshold) structure — size→gap correlation +0.57 at fixed cell |
...
**Next, in order of value:** (0) a renewal hazard — the event rate as a
function of the stress reloaded since the last event (or equivalently a
per-trajectory threshold drawn at each event), fitted from the Figure 21
gap–size relation and run forward; the aftershock term already built
(`sim_aftershock.py`) should be kept, it is what fixes the yield peaks;
...
| [pipeline/sim_aftershock.py](pipeline/sim_aftershock.py), [pipeline/sim_fine.py](pipeline/sim_fine.py), [pipeline/sequence_test.py](pipeline/sequence_test.py) | aftershock-hazard and 42×42-grid simulations; size→gap renewal correlation (Fig. 21) |
