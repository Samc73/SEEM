...
and answers the sample-maximum objection to the ceiling. Everything here is
...
goes 1.43 → 1.32). So the data cannot yet separate "the hidden variable
relaxes with strain" from "its effect is weaker at high u", which is the
same ambiguity as Figure 15 seen microscopically. Third, the dataset
...
| Whether the hidden variable relaxes with strain or is state-dependent | **undecidable here** (3 cells sampled both early and late) |
...
above touches γ > 0.5, and Section 9's relax-or-state-dependent ambiguity
is resolved by one preparation followed to γ ≈ 2; (2) a per-step
...
| [pipeline/make_figs_single.py](pipeline/make_figs_single.py), [pipeline/make_figs_memory.py](pipeline/make_figs_memory.py) | Figures 1–15 and 16–18 |
...
| figures/fig01 … fig18 | this pass (one plot per figure; 16–18 are Section 9) |
