import os as _os
"""SEEM project deck v2 — adds steady-state slide; plain research formatting."""
...
bullets(tf, [('State and events', True),
             ...
             '22 × 22 grid on (u,τ): inner 20 × 20 on the 0.2–99.8% quantiles plus a catch-all ring'])
takeaway(s, 'The target this time is the probability distribution of yield events, P(s | u,τ) — '
            'discovered, not assumed — and everything downstream is rebuilt from it.')
...
bullets(tf, [('Protocol on the distributions', True),
             ...
             '12 cells (700–7,600 events) spanning the plane, plus unweighted-bin and wider-window variants',
             'Audit: the largest cell pushed to complexity 9 — 4,505,024 trees, 48,725 admissible — no new form appears',
             'SR proposes; the likelihood disposes (next slides)'])
...
s = slide()
title(s, 'SR proposes, the likelihood disposes')
fig_left(s, 'fig04_falsifiability.png', [
    ...
    'The ceiling law wins 8 of 8 large cells: ΔAIC 67–120 over the TPL, 300–754 over the lognormal',
...
takeaway(s, 'The bounded-support law is decided by per-event likelihood with falsifiability controls, not by the binned fit.')
...
# ------------------------------------------------------------ 10 forward sim
s = slide()
title(s, 'The acid test: simulating all 850 runs')
...
takeaway(s, 'Memory acts through event size, not event rate — against the simplest one-variable STZ reading, '
...
fig_wide(s, 'fig19_memory_relaxes.png', [
    ...
    'and it falls within fixed u bands — relaxation, not state dependence; γ_r ≈ 0.18',
...
fig_left(s, 'fig22_persample.png', [
    'Parameter-free prediction: apply the between-preparation relation within a preparation — '
...
fig_left(s, 'fig18_ceiling_pinning.png', [
    ...
    'So does a true ceiling with m ≈ 2 at n ≈ 5,000 (green synthetics: c/s_max 1.02–1.12)',
    'A true TPL under the same fit runs off to 3–350× the maximum and collapses on refit (blue)',
    'Merging consecutive drop steps into single avalanches (187k → 172k): the ceiling law wins by more '
    '(ΔAIC 117–213), s_c moves < 2%, k shifts +0.05',
...
fig_wide(s, 'fig20_clustering.png', [
    ...
    'Event counts per window are over-dispersed (Fano 1.3–1.6 vs the model’s 1.0) while the stress released per window is less variable (CV 0.10 vs 0.13)',
    'Referenced to the last large event, the large-event hazard is 1.5× higher after 0.1–0.3 of reloading; large events are more regular (Fano 0.51 vs 0.71)'], size=12.5, h=3.9)
...
bullets(tf, [('Run-to-run scatter within a preparation: model 25–30% too high', True),
...
col(4.85, 'NOT ESTABLISHED', MUTED, [
    ...
    'Convergence of preparations at large strain — the model’s opinion, now with a measured rate but no data past γ = 0.5',
...
steps = [
    ...
    ('Reverse loading', 'unchanged from the first pass — parity test for a back-stress variable'),
...
takeaway(s, 'Everything here is regenerated from the raw data by pipeline/run_all.sh (43 min); Figures 1–22 and the README carry the details.')
